...
class Model(object):
    ...
    def __str__(self):
        outstr = (
            f"Model:{self.PL0}\t{self.PLa}\n"
            + f"{self.PD0}\t{self.PDa}\n"
            + f"{self.PTxwx}\t{self.PTxwz}\n"
            + f"{self.PTy0}\t{self.PTwa}\t{self.PTywy}\n"
            + f"{self.PTzwz}"
        )
        return outstr
...
if __name__ == "__main__":
    test_model = Model(
        0.33, 1.9, 0.18, 0.69, 0.43, -1.4e-2, -8.2e-2, -1.2e-2, -1.7e-3, -3.4e-5
    )
    print(test_model)
